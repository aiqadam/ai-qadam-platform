...
> Note: PR #97 carried pre-existing CI failures on `ci` (vitest) and `ci/storybook` (rolldown JSX) jobs. Both failure classes are owned by other issues/workflows — see [ISS-TEST-WEB-001](ISS-TEST-WEB-001.md) (queued: `wf-20260703-fix-066-vitest-bump`) and [ISS-CI-002](ISS-CI-002.md) (rolldown block). This UI fix did not introduce either failure. Counter tracking lives in `.copilot/meta/ci-override-counters.json`.
