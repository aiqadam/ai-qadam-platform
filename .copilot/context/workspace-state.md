...
**Last updated:** 2026-07-04 — `wf-20260704-fix-093` merged. [PR #109](https://github.com/tvolodi/aiqadam/pull/109) squash `255d2bb` (ISS-CI-OVERRIDE-ebd184b — root cause: `@storybook/react-vite@8.6.18` is a pass-through adapter that does not inject any Vite plugin for JSX; rolldown 1.1.3 (transitive via `@astrojs/react` → `vite@8`) parses `.tsx` files itself with JSX disabled by default → 12 `PARSE_ERROR: Unexpected JSX expression` against `apps/web-next/src/kit/*` and `apps/blocks/workspace/AsyncSelect.tsx` during `pnpm --filter @aiqadam/storybook build`. Fix: added `@vitejs/plugin-react@^5.2.0` as a direct devDep of `apps/storybook`; injected `react({ jsxRuntime: 'automatic' })` as the first plugin in `viteFinal` so Babel transpiles JSX before rolldown's parser sees it. Build now exits 0 with 226 modules transformed (31+ asset chunks including per-atom stories). Counter for failure class `ebd184bfe1c7b3c4fde6d4d0685be02d595d8be7` reset to 0 per AGENTS.md §6.3 step 5 (PR merged with green storybook CI, no override applied). Counter bumped 93 → 94. Auto-merge per AGENTS.md §6.2 + §6.3 user CI opt-out. Predecessor `wf-20260704-fix-092` (PR #108 squash `69f2b3f`).
...
_(none — `wf-20260704-fix-093` has merged. Next to pick up is one of the queued follow-up workflows below, in priority order.)_
