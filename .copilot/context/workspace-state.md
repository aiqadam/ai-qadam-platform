...
**Last updated:** 2026-07-04 — `wf-20260704-fix-092` merged. [PR #108](https://github.com/tvolodi/aiqadam/pull/108) squash `69f2b3f` (ISS-UAT-BATS-001 — `scripts/tests/uat-seed.bats` row 6 (FR-WORKFLOW-003 row 6): pinned baseline to immutable SHA `8db37ac^` (Bug A: `origin/main:` fallback no longer pre-fix after main advanced past `2b72f46`); replaced strict byte-equality with structural assertion + drift-whitelist (Bug B: too strict for `wf-20260704-fix-086`'s documented `@aiqadam.test → @example.com` TLD migration). bats **34/34 pass** (was 33/34). Per `AGENTS.md §14` authorization the Orchestrator registered the dedicated `ISS-UAT-BATS-001.md` in the same workflow, following the `ISS-PREEX-001` precedent (severity: minor, module: uat/test-design) — this closes a loop that 3 prior PRs (#105, #106, #107) had disclosed as "owned by follow-up `wf-20260704-fix-087-fix-fr-workflow-003-row-6`" but never actually queued. Counter bumped 92 → 93. Auto-merge per AGENTS.md §6.2 + §6.3 user CI opt-out. Predecessor `wf-20260704-feat-090` (PR #107 squash `c013f6e`) — see Completed Workflows (recent) table below.
