...
(PR [#268](https://github.com/aiqadam/ai-qadam-platform/pull/268), squash-merged `<pending>`):
`wf-20260804-feat-208` (PR #260, merged `4edb1c4`) shipped FR-AUTH-007's API (`telegramLinked`/`telegramUsername` on `TelegramMeResult`) and web (`LinkedAccountsPanel`) surfaces, but the matching bot-side `/me` change was only ever written to the local working tree — never committed, pushed, or included in that PR, despite the workflow's own summary claiming it was done. Found as a dirty `apps/bot` submodule during an unrelated session (`wf-20260804-fix-209`) and resolved here: bot commit [`aiqadam-telegram-bot@ed2228c`](https://github.com/aiqadam/aiqadam-telegram-bot/commit/ed2228c) makes `/me` render the caller's real Telegram link state (`"@handle — Telegram linked"` or a `/link` prompt) instead of the old static "link on web" CTA. Along the way: moved the hardcoded strings the found diff contained into the `en`/`ru` locale tables (matching every other line in the file), deleted the now-dead `me.link_web_cta` key, and corrected `me.py`'s module docstring (which described the old "no linked-status concept exists" scope decision as still current). `FR-AUTH-007.md`'s AC-5 flipped to verified with an explicit scope note: the bot's "linked providers summary" is Telegram-only, since `TelegramAuthService` has no read path to Email/Google/GitHub link state (Authentik-only, web-side). `apps/bot`: 212/212 tests pass (5 new: 3 render-path + 2 API-client parsing), `ruff check`/`ruff format --check` clean on every changed file. `business_process: []` for this workflow specifically (checked `BP-UAT-003.md` directly — it's entirely the web `/me/profile` cabinet, zero bot-surface steps; the FR-level linkage is unchanged).

**Prior last updated:** 2026-08-04 — `wf-20260804-fix-209-fast-uri-supply-chain` (merged).
