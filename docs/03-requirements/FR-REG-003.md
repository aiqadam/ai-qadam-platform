...
4. **Cancellation window** — Cancellation is allowed up until the event ends (`ends_at`). After the event, registrations are locked and status transitions are blocked (except for `no_show` marking by the system).
5. **Cancellation from bot** — Bot `/cancel N` command (FR-BOT-002) calls the same endpoint via the internal API path.
6. **Notification** — No notification sent to the cancelling user (they initiated it). A notification is sent to the promoted waitlist member (see FR-REG-002).
