...
> Restructured per `docs/critical-review.md`. Old sequential sprint-by-sprint plan was wrong for community-building at scale with limited core team effort: too sponsor-heavy too early, missing member-to-member layer, missing acquisition mechanics, missing volunteer/ambassador architecture, no defined success metric.
>
...
- [ ] **7.4 — Cross-channel Telegram identity reconciliation**
  ...
  - **Acceptance:** all three scenarios pass an integration test against staging Authentik + bot + web; explicit edge-case docs in `docs/auth-architecture.md`.
