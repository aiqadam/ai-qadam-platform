...
> **Status:** Research note, not a commitment. Forum work is deferred per [ADR-0037 §Defer-list](./adr/0037-three-tier-architecture.md) and [product-plan §6.2](./product-plan.md). This brief is the playbook you reach for when the entry gate fires.
>
...
- **Don't deploy yet.** Entry gate is `~200 daily-active members` per [product-plan §6.2](./product-plan.md) — that gate is not currently met.
...
| Build in-house | ❌ Skip | ~5 sprints of inferior work per [critical-review §12](./critical-review.md). Member-to-member discussion is not where our domain expertise sits. |

### Why not deploy now

- **No critical mass.** [product-plan §6.2](./product-plan.md) gates Discourse on `~200 daily-active members`. Below that threshold the forum sits empty, which actively damages perception ("AI Qadam is dead" → opposite of the goal). Deploy when there are enough members that any given visitor finds activity within the past 24h.
...
- **Ops ceiling.** Prod host RAM is finite; the Discourse stack adds ~500MB. Per [interaction-architecture §OPS](./interaction-architecture.md#sprint-9--operator-tools--polish-3-prs), we cap at 12GB used. Check current headroom before adding another container.
...
- **Outbound intent** (dispatcher → Discourse):
  ...
  - Wired into the existing `Interactions dispatcher` per [interaction-architecture §3.4](./interaction-architecture.md)
...
Per [interaction-architecture Phase 2 advance gate](./interaction-architecture.md):
...
- [product-plan §6.2](./product-plan.md) — 200 DAU gate
- [community-platform-roadmap §6 (Phase ζ)](./community-platform-roadmap.md) — original Sprint 6.2 plan
- [interaction-architecture §3.4](./interaction-architecture.md) — webhook + dispatcher wiring detail
- [critical-review §12](./critical-review.md) — the "Discourse vs build-it" decision
