...
| Item | Output | Effort |
|---|---|---|
| **3.0 — Member graph foundation** | Per [ADR-0033](./adr/0033-community-member-graph.md) Part 1: bootstrap.sh extensions — extend `directus_users` (job_title, employer FK, seniority enum, industry tags, is_student, appear_in_directory), new collections `member_skills`, `member_employments`, `member_interests`, `member_consents`, `member_connections`, `companies`, `cohorts`, `partner_audiences`. Event taxonomy extension on existing `events` (visibility enum, audience_cohort_id, price_usd, capacity_band). `event_outcomes` + `event_followups` denormalised rollup. Idempotent ensure() helpers. **Blocks all subsequent Sprint 3 cabinets.** | 1 vertical PR |
| **3.1 — Single-origin cabinet routing ADR** | Per architect review: decide explicitly between separate-subdomain SSO maze vs single-origin role-routed (`workspace.aiqadam.org/<concern>`). Architect picks single-origin (already implemented via `/workspace/*` per ADR-0032 acceleration). ADR documents reasoning + locks future cabinet structure. | 0 code (ADR) |
...
| **3.3 — Cabinet #2: Announcement composer** at `/workspace/announce` | Country lead + board personas. Pick cohort → write message → preview rendered email → send via Interactions dispatcher (already shipped). Activates the cohorts built in Cabinet #1. | 1 vertical PR (~1 day) |
