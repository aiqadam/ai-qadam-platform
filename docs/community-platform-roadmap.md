...
| **0.6 — RBAC manifest ADR** (no code) | `docs/adr/0021-rbac-manifest.md` defining: roles inventory, Authentik groups → engine permissions mapping, sync trigger (webhook vs poll), conflict resolution rules, partial-failure handling. Blocks Sprint 2.2. **Accepted 2026-05-21** (refreshed post-batch-#1 to drop Twenty per ADR-0033 + reference ADR-0031 cabinet routing + acknowledge F-S0.2 break-glass shipped). Unblocks F-S2.2 / 2.4 / 2.5 / 2.6 + the F-#113 `rbac.denied` hook. | 0 code, just docs (shipped) |
...
| **RBAC manifest** | F-S2.2 sync service, S2.4/2.5/2.6 cabinets, F-#113 `rbac.denied` hook | PM + Architect | **Accepted 2026-05-21** per [ADR-0021](./adr/0021-rbac-manifest.md) (refreshed post-batch-#1). 7-role inventory + Authentik-as-source-of-truth + Directus + Plausible engines (Twenty removed per ADR-0033). Unblocks all gated Sprint-2 features. |
...
| 2026-05-21 | ADR-0021 RBAC manifest Accepted (refreshed for Twenty removal + ADR-0031/0033 references + F-S0.2 break-glass resolution). Unblocks F-S2.2 / 2.4 / 2.5 / 2.6 + F-#113 `rbac.denied` hook. | PR #160 |
