...
| **1.4 — Pre-event reminder cron** | Cron: T-7 days → speakers brief refresh; T-2 days → registrants reminder + "bring a question" prompt; T-3 hours → final reminder + venue + cancel link. Routes via preferred channel (Telegram > email when bot is linked). Idempotent (skip if already sent). **F-S1.4 SHIPPED 2026-05-22** — `POST /v1/internal/event-reminders/tick` (InternalAuthGuard) called by external scheduler every ~10 min; finds events in T-2 window `[38h,58h]` + T-3h window `[2h,4h]`; dispatches `reminder_72h` / `reminder_3h` to `status IN (registered, attended)` attendees; idempotent via `event_announcements` kind=`reminder_t_minus_2`/`_3h`; consentBasis=operational_contract (service-level). 6 unit + 2 smoke tests. Runbook `docs/runbooks/event-pre-event-reminders.md` (covers wiring options: GH Actions / Coolify / systemd). T-7 speaker brief deferred to F-S1.4b once F-S1.1b adds the speakers schema. Telegram-channel preference defers to F-S5.5 bot integration. | 1 PR shipped (T-2 + T-3h attendee paths) + follow-ups for T-7 speaker + Telegram routing | [§4.1](#41-member-onboarding-ceremony-registration--first-event) |
...
| **1.6 — Lead capture for non-registrants + 3-email nurture** (Marketing addition) | Per [marketing playbook §3.2](./marketing-and-pr-playbook.md#32-activation--they-take-the-key-action): visitors who give email but don't register today (e.g., "notify me when next event in {city}") get the `lead` user state + a 3-email nurture: T+0 welcome / T+3 "here's why community matters" / T+7 next event preview. Uses Interactions dispatcher. CTA on homepage + event detail + /events pages: "Notify me of new events in {city}." **Capture + verify + convert shipped 2026-05-21** — `directus_users` extensions (state enum, email_verified, city, interest_topics, source_url, acquisition_source), `LeadsService` (POST /v1/leads + GET /v1/leads/verify + auth-callback lead→member upgrade with `lead_converted_to_member` dispatch), `LeadCaptureForm` island embedded on `/` and `/events`, `/leads/{thank-you,verified,verify-failed}` pages, honeypot + UTM first-touch capture. 10 unit tests on the service. Operator targeting works today via `/workspace/members` cohort builder (`state=='lead'` etc.). T+3/T+7 nurture cron flows deferred to **F-S1.6b** (need Directus cron-flow spec); see `docs/runbooks/lead-nurture.md`. | 1 PR shipped (capture + verify + convert); 1 PR pending (F-S1.6b nurture flows) | [§3.1 member lifecycle](#31-member-lifecycle) (lead state) |
...
| **1.4** pre-event reminders (T-2 + T-3h shipped) | Tick `/v1/internal/event-reminders/tick` → events in T-2 window get `reminder_72h` to registered attendees; events in T-3h window get `reminder_3h`; second tick within window = no-op (ledger row). T-7 speaker brief deferred until F-S1.1b ships the speakers schema. |
...
| **1.6** lead capture (shipped) | Submit lead form on /  or /events → 202 + verify email arrives; click link → /leads/verified renders + `directus_users.email_verified=true`; subsequent Authentik sign-in for same email → state flips lead→member + `lead_converted_to_member` dispatched. F-S1.6b nurture cron flows still pending. |
