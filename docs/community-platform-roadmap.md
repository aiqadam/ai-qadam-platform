...
| **1.5 — Pre-event member-to-member matching** (BA addition) | T+3 days post-registration OR T-7 days pre-event: dispatch "3 people you might want to meet" message. **F-S1.5 SHIPPED 2026-05-22** (T-7): `POST /v1/internal/event-matches/tick` finds published events in [now+6.5d, now+7.5d]; ranks by interest-tag overlap; `appear_in_matches` opt-out. **F-S1.5b SHIPPED 2026-05-22** (T+3 + job-title): `POST /v1/internal/event-matches-post-reg/tick` (InternalAuthGuard, hourly) finds registrations with `date_created <= now-3d` AND event still > 7d out; ranks by `shared_tags*2 + (same_job_title ? 1 : 0)`; new `member_match_dispatches(user, event, kind)` collection provides per-(user, event) idempotency shared with T-7 — mutually exclusive, whichever fires first wins. 14 unit + 4 smoke tests across both PRs. Job-title match is exact-string post-normalize; controlled-vocab taxonomy + `member_connections` history de-prioritisation deferred. | 2 PRs shipped (T-7 + T+3 with job-title) | [§4.1](#41-member-onboarding-ceremony-registration--first-event), [§6 risk #6](#6-behavioral-risks--mitigations) |
...
| Item | Output | Effort |
|---|---|---|
| **4.1 — Country provisioning service** — **PR-1 (framework) SHIPPED 2026-05-23** | State-machine-backed (per architect review). On super-admin trigger, sequentially: register Authentik OIDC redirect URI → create Directus permission policy (member-graph-aware per ADR-0033) → create Plausible site → register Coolify FQDN. Each step idempotent + retriable. State persisted in `countries.provisioning_state` JSON. **F-S4.1-a (this PR)**: framework — schema column, `CountryProvisioningService` with the 4-step state machine, `POST /v1/admin/countries/:code/provisioning/run` (idempotent, resumes from first non-succeeded step) + `GET /v1/admin/countries/:code/provisioning` (super-admin gated), 9 unit tests covering init/persist/resume/idempotency. v1 step runners are STUBS that log intent + succeed — real integrations slot in via subsequent narrow PRs (F-S4.1-b Authentik, -c Directus policy, -d Plausible + Coolify). Operator UI (F-S4.2 wizard) can build against the state contract now. | 3 PRs (1 of 3 shipped — framework; integrations pending) |
