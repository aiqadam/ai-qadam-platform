...
| **0.4 — Observability v0** | Loki + Promtail for log aggregation (~1.5 GB RAM); Uptime Kuma probing every public endpoint with alerts → Telegram/email; Plausible custom events for auth failures, dispatch failures, RBAC denials. | 1 PR + Coolify config |
