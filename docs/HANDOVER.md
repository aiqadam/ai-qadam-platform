...
When you evaluate any new dependency or service, the first filter is "does this add a recurring bill?" If yes, it almost certainly needs the owner's explicit sign-off. The deferred money/hiring decisions are tracked in [`docs/business-process-gaps.md`](business-process-gaps.md).

### 1.4 North-star metrics (what "good" means)
From [`docs/community-platform-roadmap.md`](community-platform-roadmap.md) §1. The headline ones:
...
Every new feature is supposed to run through a **layer-triage** step before anything else (see [`docs/agent-prompts.md`](agent-prompts.md) §"Layer triage"). The web V2 rewrite is the "Phase A rewire" that this ADR gates; a lot of Sprint 4 + later work was deliberately *deferred* behind it.
...
- Before writing any V2 code: read the ADR + [`docs/architecture/blocks.md`](architecture/blocks.md), search for an existing block, and either reuse it or open a **block-proposal PR first** (block + Storybook story + catalogue entry).
...
- **Backups / DR:** hourly restic snapshots (Coolify + Directus + Authentik + platform DB) → Cloudflare R2; Backrest UI at `ops.aiqadam.org` (super-admin only via Authentik forward-auth). **First move for any "I broke prod" scenario** → [`docs/runbooks/snapshot-restore.md`](runbooks/snapshot-restore.md).
...
- **Cron:** no external schedulers. Recurring in-platform work uses `@Cron(...)` + `TickLockService.withLock(name, ttlSec, fn)` for multi-replica safety. Runbook: [`docs/runbooks/internal-cron.md`](runbooks/internal-cron.md).
...
- **Telegram (platform side):** outbox pattern with a stable `delivery_key` and a documented [delivery contract](architecture/telegram-outbox-delivery-contract.md). The bot is **acquisition-first**: `/register` must work *without* `/link` first — the bot creates community members using Telegram as an IdP. (Framing it as "link-first" has been rejected repeatedly — it's friction.)
...
The baseline (parameterized queries, input validation at boundaries, no secrets in logs/commits, output encoding, auth at controller level) is defined in [`.claude/SECURITY.md`](../.claude/SECURITY.md) and largely honored.
...
There's a planned but **not-yet-built one-command token-rotation tool** (design at [`docs/token-rotation-tool-design.md`](token-rotation-tool-design.md)). The rotation checklist lives in [`docs/runbooks/secret-rotation-pending.md`](runbooks/secret-rotation-pending.md). **Rotating these is a launch-blocker.**

---

## 8. Deferred capabilities (the explicit "NO"s)

So the next developer doesn't re-derive decisions already made. Full reasoning in the operating memory and [`docs/business-process-gaps.md`](business-process-gaps.md):
...
Authoritative sources: [`docs/community-platform-roadmap.md`](community-platform-roadmap.md) (next-12-weeks tactical, §7 feature list), [`docs/product-plan.md`](product-plan.md) (18-month strategic), [`docs/sprint-5-to-8-plan.md`](sprint-5-to-8-plan.md), and [`docs/agent-prompts.md`](agent-prompts.md) (vertical-feature backlog + kickoff prompts). In rough priority order had work continued:

1. **Finish the web V2 rewrite** (`apps/web-next`) — port the remaining V1 surface. The complete V1 feature surface + a 149-item MUST-COVER checklist is in [`docs/architecture/web-v1-feature-surface.md`](architecture/web-v1-feature-surface.md). Note: customer-surface work shipped on V1 between 2026-05-26→29 (CMS singletons, badges, `/me` uplift, plus-ban) is **not yet in V2** — hand that doc to whoever continues V2.
...
| Item | Where | Action |
|---|---|---|
| Secrets pending rotation | §7 + [secret-rotation-pending.md](runbooks/secret-rotation-pending.md) | Rotate at launch (blocker). |
| Token-rotation tool not built | [token-rotation-tool-design.md](token-rotation-tool-design.md) | Build before the next major rotation pass. |
...
| V1→V2 parity incomplete | [web-v1-feature-surface.md](architecture/web-v1-feature-surface.md) | 149-item checklist; V1 work from 2026-05-26→29 not yet in V2. |
...
These come from [`.claude/CLAUDE.md`](../.claude/CLAUDE.md) and [`.claude/WORKFLOW.md`](../.claude/WORKFLOW.md). Non-negotiable:
...
1. Read, in order: [`.claude/CLAUDE.md`](../.claude/CLAUDE.md) → [`PROJECT.md`](../.claude/PROJECT.md) → [`ARCHITECTURE.md`](../.claude/ARCHITECTURE.md) → [`STANDARDS.md`](../.claude/STANDARDS.md) → [`WORKFLOW.md`](../.claude/WORKFLOW.md) → [`SECURITY.md`](../.claude/SECURITY.md) → [`AI_COLLAB.md`](../.claude/AI_COLLAB.md) → [`GLOSSARY.md`](../.claude/GLOSSARY.md).
2. Then this doc → [`docs/product-plan.md`](product-plan.md) → [`docs/community-platform-roadmap.md`](community-platform-roadmap.md) §7 → [`docs/agent-prompts.md`](agent-prompts.md).
3. Skim the **runbooks** index ([`docs/runbooks/README.md`](runbooks/README.md)) — that's where the operational truth lives.
...
**Strategy & planning** — [`docs/product-plan.md`](product-plan.md), [`docs/community-platform-roadmap.md`](community-platform-roadmap.md), [`docs/sprint-5-to-8-plan.md`](sprint-5-to-8-plan.md), [`docs/agent-prompts.md`](agent-prompts.md), [`docs/marketing-and-pr-playbook.md`](marketing-and-pr-playbook.md), [`docs/ux-and-content-guidelines.md`](ux-and-content-guidelines.md), [`docs/business-process-gaps.md`](business-process-gaps.md).

**Architecture** — [`docs/adr/`](adr/) (0001–0038; start at 0002, 0013, 0016, 0032, 0033, 0034, 0037, 0038), [`docs/architecture/`](architecture/) (blocks, parity-matrix, web-migration-plan, web-next-kickoff, web-v1-feature-surface, wiring-map, telegram-outbox-delivery-contract), [`docs/auth-architecture.md`](auth-architecture.md), [`docs/interaction-architecture.md`](interaction-architecture.md).
...
**Security** — [`docs/platform-hardening-assessment-2026-05-29.md`](platform-hardening-assessment-2026-05-29.md), [`.claude/SECURITY.md`](../.claude/SECURITY.md), [`docs/policies/`](policies/).
