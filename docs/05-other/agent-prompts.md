...
|---|---|
| [`docs/01-business/product-plan.md`](../01-business/product-plan.md) | Strategic 18-month view: vision, surface map (web/Telegram/email/earned), member journey, platform pillars, scaling + enrichment strategy, product menu (hackathons / HRtech / edtech / paid / mentorship), business model evolution, metrics. Read when a strategic question surfaces ("does this fit?", "what comes after?"). |
| [`docs/01-business/community-platform-roadmap.md`](../01-business/community-platform-roadmap.md) | §7 lists every feature with deliverables. §1 north-stars, §2.5 3-lane model, §3 actor lifecycles, §6 risks, §7.5 smoke catalog. |
| [`docs/04-development/design-system/ux-and-content-guidelines.md`](../04-development/design-system/ux-and-content-guidelines.md) | All user-visible copy. §9 form structures, §10 task flows, §11 onboarding scripts, §12 notification copy library, §13 microcopy, §15 empty/error states. Verbatim — do not invent. |
...
`apps/web/` is grandfathered and exempt. `apps/web-next/` is the locked greenfield build (see [ADR-0038](../adr/0038-web-4-layer-architecture.md) and [`docs/04-development/frontend/web-next-kickoff.md`](../04-development/frontend/web-next-kickoff.md)).
...
2. Read [`docs/04-development/architecture/blocks.md`](../04-development/architecture/blocks.md) (the L3 block catalogue).
...
8. If the PR adds/edits a block, update `docs/04-development/architecture/blocks.md` in the same PR (Lock #4 — arch-check enforces).
9. If the PR changes data wiring, update `docs/04-development/architecture/wiring-map.md` in the same PR.
...
| **F-S0.2** | Break-glass admin path | ✅ shipped | `scripts/provision-break-glass.sh` (idempotent rotation tooling for the Directus side) + Postgres `aiqadam_breakglass` manual rotation procedure in the runbook + filled-out [`break-glass.md`](../04-development/security/runbooks/break-glass.md). API-endpoint version deferred until F-S2.2 RBAC sync. |
...
| **F-S0.11** | Prod-probe smoke (cron in place via PR #120; this adds the scheduled-failure alerting) | ✅ shipped (#151); **removed 2026-07-26** | Cron failure emits Plausible `prod_probe_failure` event + opens GH issue (`prod-probe-failure` label); recovery closes the issue. Removed because it never actually fired a real scheduled run (GitHub Actions trigger-registration bug) and its deploy-race rationale was obsoleted by the Coolify removal (#45); see `apps/e2e/README.md`'s "History" section. |
...
~~F-S1.1a publication broadcast~~ ✅ shipped 2026-05-22 · ~~F-S1.1b speaker_added flow~~ ✅ shipped 2026-05-22 (event_speakers junction + `PATCH .../speakers/:id` fires `speaker_added` on accepted→confirmed; per-(event, speaker) idempotency in `event_announcements.speaker` FK; OG-image regen + web UI deferred — operator-API + dispatch only) · ~~F-S1.1c post-event cron~~ ✅ shipped 2026-05-22 (`POST /v1/internal/post-event/tick` dispatches `speaker_thanks_with_referral_ask` + `next_event_teaser` then sets `events.post_event_processed=true`; CSAT dispatch deferred until per-recipient template renderer lands) · ~~F-S1.2+1.3 CSAT capture + operator surface~~ ✅ shipped 2026-05-22 · ~~F-S1.4 pre-event reminders (T-2 + T-3h)~~ ✅ shipped 2026-05-22 + ~~F-S1.4b T-7 speaker brief~~ ✅ shipped 2026-05-22 (`POST /v1/internal/event-speaker-briefs/tick`; per-(event, confirmed speaker) personal brief with talk title + venue + current registered count; idempotency via `event_announcements` kind=`reminder_t_minus_7_speaker` + speaker FK) · ~~F-S1.5 member matching (T-7)~~ ✅ shipped 2026-05-22 + ~~F-S1.5b T+3 + job-title overlap~~ ✅ shipped 2026-05-22 (`POST /v1/internal/event-matches-post-reg/tick`; per-registration trigger when event still > 7d out; shared algorithm with job-title boost; new `member_match_dispatches` collection makes T-7 + T+3 mutually exclusive per (user, event); controlled-vocab job-title taxonomy + `member_connections` history dedup deferred) · ~~F-S1.6 lead capture~~ ✅ shipped 2026-05-21 + ~~F-S1.6b nurture cron~~ ✅ shipped 2026-05-22 (`POST /v1/internal/lead-nurture/tick`; T+3 `lead_nurture_value` + T+7 `lead_nurture_next_event`; idempotency via new `lead_nurture_dispatches` collection; topic-personalised + city-scoped + churned re-engagement deferred to follow-ups).
...
- docs/05-other/agent-prompts.md §0–§3 (model, canonical docs, concurrency, gates)
- docs/01-business/community-platform-roadmap.md §7 — find the row for <feature-id> and read its deliverables
- docs/04-development/design-system/ux-and-content-guidelines.md — for any user-visible copy
- docs/02-business-processes/marketing-and-pr-playbook.md — for any funnel/brand/UTM touch
