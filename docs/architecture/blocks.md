...
| `<MembersList>` | `@/blocks/workspace` | _(no props — owns pagination + search + filter state, reads via `useMembersSearch()`; composes `<MembersFilterPanel>`)_ | `pages/workspace/members/index.astro` (PR 2.2) | Storyless — interactive island needs provider mocks | `/v1/workspace/members` |
...
| `<EventEditForm>` | `@/blocks/workspace` | `eventId: string` (loads via `useWorkspaceEvent`, PATCHes via `useUpdateEvent`; survey-form picker fed by `useWorkspaceForms`) | `pages/workspace/events/[id].astro` (M2.2a) | Storyless — interactive island needs provider mocks | `/v1/workspace/events/:id` (GET + PATCH) |
...
| `<AccountChip>` | `@/blocks/common` | _(no props — React island; reads `useAuth()`; avatar + role-gated popover (Workspace / Engineering Deck) + sign-out via `lib/sign-out`)_ | `<AppNav>` (M0.2) | Storyless — interactive island needs provider mocks | `useAuth()` + `/api/v1/auth/sign-out` |
