...
| `<AnnounceComposer>` | `@/blocks/workspace` | _(no props — owns cohort + subject + body state, reads cohorts via `useCohorts()`, posts preview via `usePreviewAnnounce()` from `lib/use-announce`; M2.4-i ships compose + preview, M2.4-ii adds send + delivery summary)_ | `pages/workspace/announce/index.astro` (M2.4-i 2026-05-30) | Storyless — interactive island needs provider mocks | `/v1/workspace/cohorts` (GET) + `/v1/workspace/announce/preview` (POST) |
