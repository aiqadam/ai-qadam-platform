...
| `<SavedCohortsPanel>` | `@/blocks/workspace` | _(no props — read-only horizontal list of saved cohorts, reads via `useCohorts()` from `lib/use-cohorts`; renders above MembersList in the Members cabinet)_ | `pages/workspace/members/index.astro` (M2.3b-i 2026-05-30; M2.3b-ii adds click-to-load + SaveCohortModal) | Storyless — interactive island needs provider mocks | `/v1/workspace/cohorts` |
