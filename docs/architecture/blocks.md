...
| `<SavedCohortsPanel>` | `@/blocks/workspace` | `onLoadCohort?: (cohort) => void` (horizontal list of saved cohorts, reads via `useCohorts()` from `lib/use-cohorts`; cards are non-interactive divs by default, become buttons when `onLoadCohort` is passed). M2.3b-iii relocates this from a sibling island in the page Astro to a sub-component nested inside `<MembersList>` so click-to-load can share React state via `parseDirectusToMemberFilters`. | `<MembersList>` (M2.3b-i 2026-05-30, nested + clickable in M2.3b-iii) | Storyless — interactive island needs provider mocks | `/v1/workspace/cohorts` |
...
| `<EventEditForm>` | `@/blocks/workspace` | `eventId: string` (loads via `useWorkspaceEvent`, PATCHes via `useUpdateEvent`; survey-form picker fed by `useWorkspaceForms`; blank `starts_at`/`ends_at` are OMITTED from the PATCH, not sent as `''`, per M0-fix-C) | `pages/workspace/events/[id].astro` (M2.2a) | Storyless — interactive island needs provider mocks | `/v1/workspace/events/:id` (GET + PATCH) |
