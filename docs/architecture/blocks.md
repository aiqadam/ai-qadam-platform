...
| `<CountryProvisioningWizard>` | `@/blocks/workspace` | `code: string` (super-admin cabinet; fetches via `useProvisioningState(code)` from `lib/use-provisioning`; renders the M1 `<Wizard>` atom with per-step pills + a detail strip showing status / last-attempt / error per step). M2.5-i ships read-only; M2.5-ii adds run / retry / activate; M2.5-iii adds manual-complete. | `pages/workspace/admin/countries/[code]/provisioning/index.astro` (M2.5-i 2026-05-30) | Storyless — interactive island needs provider mocks | `/v1/admin/countries/:code/provisioning` (GET) |
