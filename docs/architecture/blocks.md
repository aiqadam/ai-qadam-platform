...
| `<CountryProvisioningWizard>` | `@/blocks/workspace` | `code: string` (super-admin cabinet; reads via `useProvisioningState(code)` + writes via `useRunProvisioning(code)` / `useActivateCountry(code)` from `lib/use-provisioning`; renders the M1 `<Wizard>` atom with per-step pills + a detail strip + action footer). The footer offers Start/Re-run idempotently (POST .../run); Activate enables only when every step is `succeeded` (POST .../activate). M2.5-iii adds the manual-complete control for `awaiting_manual` steps. | `pages/workspace/admin/countries/[code]/provisioning/index.astro` (M2.5-i+ii 2026-05-30) | Storyless — interactive island needs provider mocks | `/v1/admin/countries/:code/provisioning` (GET) + `.../run` + `.../activate` (POST) |
