...
> - [`agent-prompts.md`](../05-other/agent-prompts.md) — vertical-feature backlog + concurrency rules + kick-off prompts for autonomous agent sessions.
> - [`ux-and-content-guidelines.md`](../04-development/design-system/ux-and-content-guidelines.md) — voice, forms, microcopy, notification copy library.
> - [`marketing-and-pr-playbook.md`](../02-business-processes/marketing-and-pr-playbook.md) — funnel, channels, sponsor tiers, UTM scheme.
...
Per operator clarification (2026-05-19), the platform has a **Community Volunteering Board** providing governance + advisory + sponsor-relations oversight. They read dashboards (read-only RBAC) and receive **quarterly digests** (see [marketing playbook §14](../02-business-processes/marketing-and-pr-playbook.md#14-quarterly-sponsor-digest-specification)).
...
> **2026-05-23 — Three-tier rewire active.** Per [ADR-0037](../adr/0037-three-tier-architecture.md): the platform is being reframed as three layers (engineering / operational / customer-facing). **Sprint 4 + all of Phase ζ + Sprint 1 follow-ups are DEFERRED — pending three-tier rewire**, with re-trigger conditions documented per item below. Already-shipped sprints stay shipped; bug fixes always continue. New work runs through the [layer triage](../05-other/agent-prompts.md#20-layer-triage-do-this-before-anything-else) in `agent-prompts.md` §2.0.
...
| **0.8 — UTM scheme + URL builder** (#105 merged) (Marketing addition) | Per [marketing playbook §16](../02-business-processes/marketing-and-pr-playbook.md#16-utm-scheme--attribution-standard): standardize `utm_source` / `utm_medium` / `utm_campaign` / `utm_content` conventions across all marketing links. Build a tiny URL-builder UI at `workspace.aiqadam.org/marketing/url-builder` so operators don't hand-construct UTMs. Lives in `docs/02-business-processes/marketing-utm-scheme.md`. **Without this, all marketing attribution is vibes.** | 1 PR |
| **0.9 — Brand asset library + `/press` page scaffolding** [HYBRID — CC builds scaffolding; HUMAN produces brand-judgment assets] (Marketing addition) | Per [marketing playbook §15](../02-business-processes/marketing-and-pr-playbook.md#15-brand-assets--ai-design-pipeline): new Directus `marketing_assets` collection (logos, social-card templates, photo library, video library, press-kit zip). Public `/press` page with media kit download (Binali + Viktor bios, fact sheet, logo pack). Production pipeline (Claude Design + ChatGPT Image Generator with Viktor as human-in-loop reviewer) documented in playbook. **F-S0.9b shipped 2026-05-21** — `marketing_assets` collection live on prod Directus (idempotent), `apps/web/src/lib/cms.ts` exposes `fetchMarketingAssets()` filtering on status=approved+visibility=public, `/press` Tier-2 sections (headshots / fact sheet / quarterly digest / press coverage) wired through with graceful UX §1.4 empty-state fallback. Tier 1 (logos) stays in git per [ADR-0025](../adr/0025-brand-asset-tooling.md). Page renders identically to the prior Potemkin today; lights up the moment Viktor uploads + approves the first assets. The asset-production runbook itself ships as part of [F-S0.7 operator playbook](../02-business-processes/operator-playbook/brand-asset-production.md). | 1 PR (collection + page; F-S0.9b shipped) + concurrent asset production |
| **0.10 — Browser-agent smoke test infrastructure** [CC] (3-lane reframe) | Playwright in CI + GitHub Actions workflow. Test catalog seeded from [UX guidelines §10 task flows](../04-development/design-system/ux-and-content-guidelines.md#10-task-flows): member registration, /me/preferences toggle, event detail, EULA-gated registration, profile edit, CSAT submit. Visual regression baseline established. axe-core for accessibility checks. **Every subsequent [CC] sprint item adds 1–3 smoke tests to this catalog** (see §7.5 smoke scenarios catalog). | 1 session |
| **0.11 — Production-probe browser agent** [CC] (3-lane reframe) | ~~Scheduled GitHub Actions job (every 30 min) runs Playwright against `https://aiqadam.org`~~ **Removed 2026-07-26.** Never actually ran in practice — a GitHub Actions trigger-registration bug (combining `pull_request` + `schedule` + `workflow_dispatch` in one workflow file) stuck it for months, producing zero real scheduled runs. It also predated the Coolify removal (#45, 2026-07-23), which gave `ci-cd.yml`'s deploy jobs their own inline post-deploy health check — the original deploy-race rationale for polling no longer applies. Non-deploy prod breakage (cert expiry, dependency outage) is a real remaining gap; a dedicated lightweight monitor (Gatus, already deployed at `infrastructure/gatus/`) is the better tool for that than a full Playwright suite on a cron, if it's ever prioritized. See `apps/e2e/README.md`'s "History" section. | Removed — see `apps/e2e/README.md` |
...
| **2.6 — Cross-country comparison dashboard** (BA addition) | Workspace view: events count, attendance, CSAT, sponsor count, speaker pipeline depth — country-over-country. "Top experiments to replicate" surface based on tags on `event_retrospective` field (Sprint 1.1). Healthy competition + knowledge transfer. **Marketing addition: cohort retention curves per country + K-factor (referrals per attendee × conversion rate)** per [marketing playbook §3.3, §3.4](../02-business-processes/marketing-and-pr-playbook.md#33-retention--they-come-back). | 1 PR + extensions |
...
| **3.4 — Cabinet #3: Event control panel** at `/workspace/events/[id]` **SHIPPED via F-S3.4** — list view at `/workspace/events` + detail view with phase-aware header (pre-event / live now / post-event), editable metadata (title/description/status/capacity/location), registration counts + check-in rate, 4-row followup checklist (retrospective / thank_you_sent / recap_posted / sponsor_report_delivered) with markdown notes + per-row complete toggle. Day-of phase surfaces `/checkin` scanner deep-link. 8 unit + 4 smoke tests. Runbook `docs/02-business-processes/operations/operator-event-control.md`. | Country lead + day-of organisers. Pre-event prep (speaker bench, capacity, venue, agenda), day-of check-in scanner, post-event followups (retrospective, thank-you sent, recap posted). Replaces Directus + spreadsheets for the per-event workflow. | 1 vertical PR (~2 days) |
...
| Item | Output | Effort |
|---|---|---|
| **4.1 — Country provisioning service** — **SHIPPED 2026-05-23 + revised 2026-05-24** | State-machine-backed. Per super-admin trigger: register Authentik OIDC redirect URI → create Directus per-country policy (member-graph-aware per ADR-0033) → **manual** Plausible site creation → register Coolify FQDN. Each step idempotent + retriable; state persisted in `countries.provisioning_state` JSON. **PR-a/b/c**: framework + authentik_oidc + directus_policy runners. **PR-d**: real coolify_fqdn runner. **PR-e (F-S4.1-e)**: Plausible CE v3.0.1 doesn't ship the Sites Provisioning API (maintainer confirmed in discussion #4329, no plans to add). Switched `plausible_site` runner to throw `AwaitingManualError`; new state `'awaiting_manual'` added; state machine continues past it (doesn't halt); wizard surfaces an explicit checklist + "I've done it" button; new endpoint `POST .../provisioning/steps/:stepId/manual-complete` flips the step to succeeded after operator confirmation; completed_at only set when ALL steps are `succeeded` (awaiting_manual blocks completion). `tzForCountry` helper retired from API runtime; tz map moved to wizard for the operator checklist hint. PLAUSIBLE_ADMIN_TOKEN env now unused (kept in Coolify, removal awaits launch rotation). 33 total tests in suite. | 5 PRs shipped |
| **4.2 — "Activate country" wizard in workspace** — **SHIPPED 2026-05-23 (both PRs)** | Operator UX wrapping 4.1. Per-step status display, retry button on failure, "go live" confirmation requires all green. **PR-a**: new SSR route `/workspace/admin/countries/[code]/provisioning` + island `CountryProvisioningWizard.tsx` that calls `GET /v1/admin/countries/:code/provisioning` on mount and `POST .../run` on user click. 4 step cards (status pill + per-step error + last-attempt timestamp), summary banner (not-yet/in-progress/provisioned), single Start / Resume / Retry / Re-run button (label switches by state). "Provision" link added per row in `/workspace/admin/countries`. **PR-b**: go-live gate. New endpoint `POST /v1/admin/countries/:code/activate` (AuthGuard + SuperAdminGuard) refuses with 400 `not_provisioned` when state is null / `completed_at` null, `provisioning_incomplete` when any step !== succeeded, idempotent when `is_active` already true. GET endpoint now returns `{ state, is_active }`; wizard fetches both. New "Activate (go live)" CTA appears only when all steps green AND `!is_active`; `window.confirm` before POST; on success banner flips to "Provisioned + live". 6 new unit tests (1 happy path, 1 idempotent, 3 gate refusal, 1 getStateWithActive). 30 total tests in suite. | 2 PRs shipped |
...
| **ζ.8 — Blog + RSS + posts collection** (Marketing addition) | Per [marketing playbook §5](../02-business-processes/marketing-and-pr-playbook.md#5-content-marketing-strategy--editorial-calendar): Directus `posts` collection + `/blog` Astro index + `/blog/[slug]` post template + Atom/RSS feed. Editorial calendar in Directus. Cadence: ≥ 2 posts/month minimum (event recaps + speaker spotlights). | 3 PRs |
...
These must be resolved before the named sprint can ship. **Decision-batch #1 closed 2026-05-21** (PR #149 drafted 8 ADRs; this batch flipped 7 of them via the decision-batch process — see [Decision-batch outcomes](#10-decision-batch-outcomes) below). Two ADRs (0022 country-lead compensation, 0028 first paid spend) moved to the [business-process gap list](../02-business-processes/business-process-gaps.md) because their accepted options require recurring spend that isn't authorized at this stage.

| Decision | Blocks | Owner | Status / notes |
|---|---|---|---|
| **Country-lead compensation model** | Sprint 4.3 (onboarding runbook) | PM | **Deferred 2026-05-21** to [gap list G-1](../02-business-processes/business-process-gaps.md); ADR-0022 reviewed but not accepted. Operating reality stays Option A (volunteer); revisit triggered by sustained sponsor revenue ≥ USD 15k/yr OR a candidate who can't volunteer. |
...
| **First paid marketing spend** | Year-1 growth ceiling | PM | **Deferred 2026-05-21** to [gap list G-2](../02-business-processes/business-process-gaps.md); ADR-0028 reviewed but not accepted. Capability ships per Sprint 5.9; spend itself paused until sponsor revenue stabilizes. |
| **Russian-language voice owner** | Phase ζ.6 i18n | PM | **Accepted 2026-05-21 (Option C only)** per [ADR-0029](../adr/0029-russian-voice-owner.md). Community-pool model; paid-editor half deferred to [gap list G-3](../02-business-processes/business-process-gaps.md). |
...
|---|---|---|
| 2026-05-21 | Batch #1 closed: 7 ADRs Accepted (0023 Phase-1-only · 0024 · 0025 · 0026 · 0027 · 0029 override-to-Option-C · 0030 · 0031); 2 ADRs Deferred to [gap list](../02-business-processes/business-process-gaps.md) (0022 · 0028) under the zero-recurring-spend filter | This PR |
...
### Marketing concurrent (per [`marketing-and-pr-playbook.md`](../02-business-processes/marketing-and-pr-playbook.md))
