...
| `<TgSegmentsList>` | `@/blocks/workspace` | _(no props — reads via `useTgSegments`, writes via `useCreateTgSegment`/`useUpdateTgSegment`/`useDeleteTgSegment`; inline create/edit form with live preview; uses DataTable + `<CriteriaBuilder>`) | `pages/workspace/integrations/telegram/segments/index.astro` (MIG-014) | Storyless — interactive island needs provider mocks | `/v1/workspace/tg-segments` |
