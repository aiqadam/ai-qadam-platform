...
| `<TelegramLinkStatus>` | `@/blocks/customer` | _(no props — reads via `useMyFullProfile()`)_ Read-only status section: shows "@{username} (linked)" when `telegram_user_id` is set, or "Not linked — type /link in @aiqadam_bot" otherwise | `pages/me/index.astro` (FR-AUTH-005, wf-20260803-feat-198) | Storyless — island needs provider mocks | `directus_users.telegram_user_id, telegram_username` (read-only via `GET /v1/me/profile`) |
