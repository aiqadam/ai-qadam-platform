...
| `<RegistrationCTA>` | `@/blocks/customer` | `eventId: string, capacity: number\|null, registeredCount: number, t: Translations` (React island — uses `useAuth` + `useMyRegistrationStatus` + register/cancel mutations from `lib/use-registrations`; `registeredCount` is populated by the page from `lib/api-ssr.ts`'s `fetchEventRegistrationCount()`, not `lib/cms.ts` — see ISS-EVT-005-1; `Translations.spotsTemplate`/`goingCountTemplate` are pre-rendered `{{count}}`/`{{capacity}}` template strings, not functions — `client:load` island props serialize as JSON, which silently drops function values to `null`) | `pages/events/[id].astro` (PR 1.4) | Storyless — interactive island needs provider mocks (see §Provider-coupled blocks below) | `registrations` (read via `apps/api` proxy, write) |
...
| `<LeadCaptureForm>` | `@/blocks/customer` | _(no props — anonymous React island)_ Email + city (optional) + interest topics (optional, 11 presets) + honeypot; phases: idle/submitting/success/error; submits via `apiClient` to `/v1/leads`; shows "Check your inbox" on success | `pages/index.astro` (ISS-UAT-013-3, wf-20260629-fix-035) | Storyless — interactive island | `/v1/leads` (POST) |
| `<MagicLinkForm>` | `@/blocks/customer` | _(no props — anonymous React island)_ Email only; phases: idle/submitting/success/error; submits via `apiClient` (fetch/JSON, **not** a native form — the response is always `{ ok: true }` and must never redirect the requesting browser, since the one-time link goes to the recipient's email inbox, not back to this tab); shows a "Check your email" confirmation on success | `pages/auth/sign-in-magic-link.astro` (FR-AUTH-004, wf-20260801-feat-179) | Storyless — interactive island | `/v1/auth/magic-link` (POST) |
| `<SignUpForm>` | `@/blocks/customer` | _(no props — anonymous React island)_ displayName + email + password + country + honeypot; phases: idle/submitting/success/error; submits a **native** `<form method="POST">` (not `apiClient`/fetch) so the browser follows the server's 302 → Authentik one-time-login redirect as a real navigation | `pages/auth/sign-up.astro` (ISS-USR-REG-001, wf-20260718-fix-122) | Storyless — interactive island | `/v1/auth/register` (POST, native form submission) |
| `<LinkedAccountsPanel>` | `@/blocks/customer` | _(no props — React island)_ Shows 4 sign-in provider rows (Email, Google, GitHub, Telegram) with linked state, handle display, and link/unlink actions; unlink calls `DELETE /v1/auth/linked-accounts/:provider` (409 when last method); Google/GitHub link navigates browser to `GET /v1/auth/link?provider=`; Telegram shows bot CTA only; success toast on `?linked=` query param; replaces `<TelegramLinkStatus>` (FR-AUTH-005 Phase 1) | `pages/me/index.astro` (FR-AUTH-007, wf-20260804-feat-208) | Storyless — interactive island needs provider mocks | `GET /v1/auth/linked-accounts` (read) + `DELETE /v1/auth/linked-accounts/:provider` (write) via `use-linked-accounts.ts` |
| `<ChannelToggles>` | `@/blocks/customer` | _(no props — React island)_ Email and Telegram notification master toggles; reads via `GET /v1/me/preferences/consents`, writes via `PATCH /v1/me/preferences/consents`; uses `@tanstack/react-query` for mutations; phases: idle/submitting/success/error | `pages/me/preferences.astro` (FR-NTF-005, wf-20260803-feat-205) | Storyless — interactive island needs provider mocks | `directus_users.notification_email_enabled, notification_telegram_enabled` (read+write via `/v1/me/preferences/consents`) |
| `<TopicInterests>` | `@/blocks/customer` | _(no props — React island)_ Topic interest checkboxes filtered by user's country; reads topics via `GET /v1/topics?filter[countries][_contains]=${userCountry}`, reads existing interests via `GET /v1/me/interests`, writes via `POST /v1/me/interests` + `DELETE /v1/me/interests/:id`; uses `@tanstack/react-query` for mutations | `pages/me/preferences.astro` (FR-NTF-005, wf-20260803-feat-205) | Storyless — interactive island needs provider mocks | `topics` (filtered by country) + `member_interests` (M2M, read+write) |
...
| `<MembersFilterPanel>` | `@/blocks/workspace` | `applied: MemberFilters, onApply: (next) => void, filterQuery: Record<string, unknown>, onSaveSegment: (name, type) => void, isSavingSegment: boolean` (Drawer with 7 filter primitives + "Save as segment" inline toggle → creates unified segment via `onSaveSegment` callback) | `<MembersList>` (M2.3a; FR-MIG-029 adds segment builder) | Storyless — interactive island needs provider mocks | n/a (builds the filter `<MembersList>` sends) |
| `<SavedSegmentsPanel>` | `@/blocks/workspace` | `onLoadSegment?: (filterQuery) => void` (lists unified audience segments from `/v1/admin/segments`; type badge ANN/TG/BOTH, member count, one-click load, delete via `useDeleteSegment()`) | `<MembersList>` (FR-MIG-029 2026-06-25) | Storyless — interactive island needs provider mocks | `/v1/admin/segments` (GET + DELETE) |
...
| `<PressAssetManager>` | `@/blocks/workspace` | `pressPage: PressPage, teamMembers: TeamMember[], siteSettings: SiteSettings` (React island — three sections: PressProse (pressPageSchema: heroTitle/companyBoilerplate/seoDescription/contactResponseSla/contactGuidance via `updatePressPage()`), TeamBios repeater (teamMemberSchema: name/title/role/bioMd/displayOrder; add via `createTeamMember()`, edit via `updateTeamMember()`, soft-delete via `deleteTeamMember(active=false)`), Stats (statsSchema: countriesServed/defaultDescription via `updateSiteSettings()`)) | `pages/workspace/press/index.astro` (FR-MIG-026) | Storyless — interactive island needs provider mocks | `PATCH /items/press_page` + `POST /items/team_members` + `PATCH /items/team_members/:id` + `PATCH /items/site_settings` (all Directus direct) |
| `<BadgesCabinet>` | `@/blocks/workspace` | _(no props — tabbed island: Badges tab renders `<BadgesListInner>`, Award history tab renders `<BadgeAwardHistoryInner>`; both tabs share one QueryClient via the common `<IslandRoot>` wrapper)_ | `pages/workspace/badges/index.astro` (FR-MIG-027) | Storyless — interactive island needs provider mocks | n/a (delegates to inner blocks) |
| `<BadgesListInner>` | `@/blocks/workspace` | _(no props — badge definitions DataTable with icon/name/slug/criteria/award_count columns; "Grant badge" dialog: member AsyncSelect via `searchMembers()`, badge select, optional note; POST via `useGrantBadge()`; exported for composition inside `<BadgesCabinet>`)_ | `<BadgesCabinet>` (FR-MIG-027) | Storyless — interactive island needs provider mocks | `GET /v1/admin/badges` + `POST /v1/admin/badges/grant` |
| `<BadgeAwardHistoryInner>` | `@/blocks/workspace` | _(no props — award history DataTable with per-badge `<FilterChip>` filter chips; columns: badge, member, granted-by, note, date, revoke; revoke opens confirm dialog with required reason textarea; POST via `useRevokeBadgeAward()`; exported for composition inside `<BadgesCabinet>`)_ | `<BadgesCabinet>` (FR-MIG-027) | Storyless — interactive island needs provider mocks | `GET /v1/admin/badges/awards` + `POST /v1/admin/badges/awards/:id/revoke` |
...
| `<AppNav>` | `@/blocks/common` | _(no props — sticky top nav; SSR auth hint from `Astro.locals.auth`; composes `<CountrySwitcher>` + `<LocaleSwitcher>` + Register CTA + Sign-in CTA / `<AccountChip>`; anon users see both Register → `/auth/sign-up` and Sign in buttons — GitHub #38)_ | every page via `Layout.astro` (M0.2) | Astro-only — no story | `Astro.locals.auth` (SSR) |
| `<AccountChip>` | `@/blocks/common` | _(no props — React island; self-wraps in `<IslandRoot>` (M0-fix-B) so its `useAuth()` has a provider in its own root; avatar + role-gated popover (Workspace / Engineering Deck) + sign-out via `lib/sign-out`; Engineering Deck target is `PUBLIC_AUTHENTIK_URL` (default `auth.aiqadam.org`), env-specific per `deploy/docker-compose.{prod,qa}.yml` — ISS-262)_ | `<AppNav>` (M0.2) | Storyless — interactive island needs provider mocks | `useAuth()` + `/api/v1/auth/sign-out` |
