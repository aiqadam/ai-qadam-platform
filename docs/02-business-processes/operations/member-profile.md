...
**Pre-reading:** [ADR-0033](../../adr/0033-community-member-graph.md), [marketing playbook §16.3](../marketing-and-pr-playbook.md#163-attribution-model), [interaction-architecture.md](../../04-development/architecture/interaction-architecture.md).
...
The Anon view renders inline when `/api/v1/auth/refresh` returns non-200. The redirect-to-Authentik flow is in the auth controller, not this page. Check `aiqadam-refresh` cookie presence + auth runbook (`docs/04-development/infrastructure/runbooks/auth.md`).
