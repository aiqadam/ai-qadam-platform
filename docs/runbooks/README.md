...
- **Tutorials / onboarding.** Use `.claude/AI_COLLAB.md` or the operator playbook.
- **One-shot scripts.** Put them in `scripts/` with a comment header.
- **Domain explanation.** Use `.claude/GLOSSARY.md` or the relevant doc.
...
|---|---|---|
| Infrastructure / CI / deploy / observability / backups / supply chain | `docs/runbooks/` | [Agent-Infra](../agent-prompts.md#agent-infra) |
| Authentication / RBAC / break-glass / audit | `docs/runbooks/` | [Agent-Docs](../agent-prompts.md#-agent-docs) |
| Country-lead activation / RBAC drift | `docs/runbooks/` | [Agent-Docs](../agent-prompts.md#-agent-docs) |
| Brand-asset production / sponsor pipeline / quarterly digests | `docs/runbooks/` | [Agent-Marketing](../agent-prompts.md#-agent-marketing) |
| Trust + safety / moderation / crisis comms | `docs/runbooks/` | [Agent-Docs](../agent-prompts.md#-agent-docs) — ζ.7 |
| Discourse / talk-recordings / hackathon teams | `docs/runbooks/` | Agent-Infra + Agent-API jointly — Phase ζ |
| Operator-playbook-style how-tos (event production, speaker outreach) | `docs/operator-playbook/` (not here) | [Agent-Marketing](../agent-prompts.md#-agent-marketing) |
...
- [`supply-chain.md`](./supply-chain.md) — pnpm audit + Trivy + Dependabot triage (per Sprint 0.3).
...
- [`docs/agent-prompts.md` §1](../agent-prompts.md#1-the-7-streams) — file-ownership table that determines who writes which runbook.
- [`docs/community-platform-roadmap.md` §7 Sprint 0.13](../community-platform-roadmap.md) — the Sprint 0 ask that produced this README and the planned-scaffolds list.
- [`docs/decision-batch-process.md`](../decision-batch-process.md) — how runbooks that codify a decision relate to ADRs (ADR first, runbook second).
