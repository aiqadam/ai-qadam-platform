...
| New collection / field that needs to ship | Extend `bootstrap.sh` append-only; add `ensure()` call; verify idempotency twice against prod | Open a new vertical PR per [docs/agent-prompts.md](../agent-prompts.md) §2 template |
...
- [docs/migration-to-directus-centric.md](../migration-to-directus-centric.md) — why Directus is the entity store
- [docs/interaction-architecture.md](../interaction-architecture.md) — dispatcher that consumes `cohorts`
