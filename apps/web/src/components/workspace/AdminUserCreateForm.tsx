...
// F-S2.7 (ADR-0035) + F-S2.8.2 — admin form. POST /api/v1/admin/invites
// returns { invite_id, invite_url, token_prefix, expires_at }; the URL
// contains the plaintext token shown ONCE. UX: render the URL in a
// copy-button success panel; do NOT navigate away.
//
// F-S2.8.2 — the cabinet no longer collects a destination Gmail or
// surfaces email-automation results. The operator picks their own
// destination + sees their own Resend key on /onboard. The admin's
// only output here is the invite link.
  ...
  if (state.phase === 'done') {
    return (
      <div style={panelStyle()}>
        ...
        <p style={{ ...mutedStyle(), marginTop: 24, fontSize: 13 }}>
          Share this link with the invitee via a secure channel (1Password Share recommended). When
          they open it they'll set their password, then provide their personal forwarding email
          themselves — you don't need to collect or paste anything else.
        </p>
      </div>
    );
  }
