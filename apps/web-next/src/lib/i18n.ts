...
// Server-side i18n, ported from apps/web/src/lib/i18n.ts (ISS-WEB-NEXT-I18N-001).
// React islands take translated strings via a `t` prop from their .astro
// parent rather than importing i18next directly — see AppNav.astro passing
// `t` into <AccountChip>.
//
// Bootstrapping i18next at module-load is safe because the locale files
// are bundled JSON, not loaded async. One init across the whole app.
// apps/web-next has no prerendered customer pages (astro.config.mjs
// `output: 'server'`), so every page gets per-request cookie resolution —
// no `export const prerender = false` opt-in needed, unlike legacy.
