...
// /events/<id> link. The cabinet share buttons URL-encode a link to
// the event page; this endpoint backs the og:image meta on that page.
//
// Cache: 5-minute public cache. The `[id].astro` page can inject a
...
// private title never leaks via OG preview. This check is INDEPENDENT
// of the page's session-aware `isGated` (per the impact analysis) —
// this route has no session to check against, so it 404s any
// non-public event unconditionally. A scraper preview must never be
// able to distinguish "members-only, you're not signed in" from
// "doesn't exist".
//
// Port of apps/web/src/pages/events/[id]/og-card.png.ts (FR-EVT-004
// gap-closure, wf-20260730-feat-155), reading from web-next's
// lib/cms.ts fetchEvent/fetchEventSpeakers instead of v1's.
